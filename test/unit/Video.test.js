...
describe("Video contract", function () {
  ...
  it("should add video in mapping", async function () {
    ...
    const [ipfs, title, desc, likes, dislikes, tipamount, owner] =
      await videos.getVideoDetails(1);
  ...
  it("should like the video", async function () {
    ...
    const [ipfs2, title2, desc2, likes2, dislikes2, tipamount2, owner2] =
      await videos.getVideoDetails(1);
    expect(likes2).to.equal(1);
    expect(likeHash[0]).to.equal(
      "Qmeff4AvGELYTWUZD23s36VrPo6An24wywFxoDskjzVPJc"
    );
  ...
  it("should dislike the video", async function () {
    ...
    const [ipfs3, title3, desc3, likes3, dislikes3, tipamount3, owner3] =
      await videos.getVideoDetails(1);
    expect(dislikes3).to.equal(1);
    expect(dislikesHash[0]).to.equal(
      "Qmeff4AvGELYTWUZD23s36VrPo6An24wywFxoDskjzVPJc"
    );
  ...
  it("should tip video owner with 1 ether", async function () {
    ...
    const [ipfs4, title4, desc4, likes4, dislikes4, tipamount4, owner4] =
      await videos.getVideoDetails(1);
